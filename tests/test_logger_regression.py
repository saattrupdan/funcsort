...
class TestLoggerRegression:
    ...
    def test_blank_line_between_imports_and_module_level_call(self) -> None:
        """Regression: statements after imports need a blank-line separator.
...
"""
        ...
        for i, line in enumerate(lines):
            if "basicConfig" in line:
                ...
                assert lines[i - 1] == "", (
                    f"no blank line before basicConfig at {i}, got {lines[i - 1]!r}"
                )
                # Line before the blank should be an import (not blank)
                assert "import" in lines[i - 2], (
                    f"no import before blank at {i - 2}, got {lines[i - 2]!r}"
                )
                break

    def test_logger_before_main_constant_dependency(self) -> None:
...
"""
        ...
        assert logger_idx < main_idx, (
            f"logger must precede main(): logger {logger_idx}, main {main_idx}"
        )

    def test_logger_before_main_with_helper_function(self) -> None:
...
"""
    ...
    def test_main_before_functions_it_calls(self) -> None:
...
"""
        ...
        helper_idx = result.find("def _helper()")
        assert main_idx < helper_idx, (
            f"main must precede _helper: main {main_idx}, _helper {helper_idx}"
        )


def test_hun_sum_mini_sorted_correctly() -> None:
    """Real-world uv script: functions sorted by call hierarchy, safely.

    Conservative contract: the header, docstring, imports, constants and the class
    all keep their original positions. Only the trailing block of module-level
    functions is reordered by call hierarchy - the entry point ``main`` first, then
    the functions it calls (top-down) - and the result stays valid Python.
    """
    before = """# /// script
...
"""
    ...
    # The result must still be valid Python (no forward references broken).
    compile(result, "hun_sum_mini", "exec")

    # Anchors (header line, docstring, imports, constants, the class) are untouched.
    for anchor in [
        "# /// script",
        '"""Create the Hungarian summarisation dataset based on hun-sum-chatml-5k."""',
        "import json",
        ...
        assert result.count(anchor) == 1, f"anchor changed: {anchor!r}"

    # Constants keep their positions relative to the surrounding definitions.
    assert result.index("CACHE_FILE =") < result.index("class SummaryValidation")
    assert result.index("def load_cache") < result.index("summary_cache = load_cache()")
    assert result.index("summary_cache = load_cache()") < result.index("def main(")
    assert result.index("client = OpenAI") < result.index("def main(")

    # The load_cache / save_cache block keeps its original relative order.
    assert result.index("def load_cache") < result.index("def save_cache")

    # The main() run is ordered by call hierarchy: entry point first, then the
    # functions it calls (create_splits and process, then process's callee).
    assert result.index("def main(") < result.index("def create_splits(")
    assert result.index("def create_splits(") < result.index("def process(")
    assert result.index("def process(") < result.index("def _text_summary_alignment(")

    # The __main__ guard stays at the very end.
    assert result.index("def _text_summary_alignment(") < result.index(
        'if __name__ == "__main__":'
    )
