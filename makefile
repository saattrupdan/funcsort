...
install: ## Install dependencies
	@echo "Installing the 'funcsort' project..."
	@$(MAKE) --quiet install-uv
	@$(MAKE) --quiet install-dependencies
	@$(MAKE) --quiet setup-environment-variables
	@$(MAKE) --quiet setup-git
	@$(MAKE) --quiet add-repo-to-git
	@echo "Installed the 'funcsort' project! You can now activate your virtual environment with 'source .venv/bin/activate'."
...
add-repo-to-git:
	...
	@if [ "$(shell git remote)" = "" ]; then \
		git remote add origin git@github.com:saattrupdan/funcsort.git; \
	fi
...
docker:  ## Build Docker image and run container
	@make install
	@docker build -t funcsort .
	@docker run -it --rm funcsort
